...
const CatalogViewer: React.FC<CatalogViewerProps> = ({ 
    ...
    const isVisible = (path: string): boolean => 
        viewerState.current?.isVisible(path) ?? true;

    const isExpanded = (path: string): boolean => 
        viewerState.current?.isExpanded(path) ?? false;

    const hasMatchingChildren = (path: string): boolean => 
        viewerState.current?.hasMatchingChildren(path) ?? false;
    ...
    const handleGenerateQuery = (
        ...
        if (queryType === 'SELECT' && tableRef) {
            ...
            SchemaProvider.getTableWithCache(tableRef, (table : any) => {
                const columns = table.getColumns().map((col: { getName: () => string }) => col.getName()).join(',\n    ');
        ...
        <div className="catalog-viewer">
            <div className="catalog-viewer-header">
                ...
                <button 
                        ...
                        title="Reload Catalogs"
                        onClick={loadCatalogs}
                        disabled={isLoading}
                    >
                        &#x27F3;
                </button>
                <label className="search-columns-toggle" title="Enabling will fetch table columns for search which may take additional time.">
                    <input
                        type="checkbox"
                        ...
                        return (
                            <div key={catalogName} className="catalog-section">
                                <div className="viewer_catalog" onClick={() => handleToggle(catalogPath)}>
                                    ...
                                    <span className={`expand-indicator ${!isExpanded(catalogPath) && hasMatchingChildren(catalogPath) ? 'expand-indicator-has-matches' : ''}`}>
                                ...
                                {isExpanded(catalogPath) && (
                                    <div className="viewer_catalog_body">
                                        ...
                                        {Array.from(catalog.getSchemas().values())
                                            .sort((a, b) => a.getName().localeCompare(b.getName()))
                                            .map(schema => {
                                                const schemaPath = buildPath.schema(
                                                    catalogName, 
                                                    schema.getName()
                                                );
