...
interface SchemaProps {
    ...
    onGenerateQuery?: (queryType: string, tableRef: TableReference | null, catalogName?: string, schemaName?: string) => void;
}
...
const CatalogViewerSchema: React.FC<SchemaProps> = ({ 
    ...
    return (
        <div className="schema-section">
            <div 
                className="viewer_schema" 
                title="Schema"
                onClick={() => onToggle(schemaPath)}
            >
                <span className="schema-name">{schema.getName()}</span>
                <span className="helper-text">schema</span>
                <span className={`expand-indicator ${!isExpanded && hasMatchingChildren(schemaPath) ? 'expand-indicator-has-matches' : ''}`}>
                    {isExpanded ? '▼' : '▶'}
                </span>
